...
end-to-end 6가지. 각 프롬프트는 Claude Code(`/skill`) 또는 Codex(`$skill`)에서 그대로 동작합니다.
...
```text
$kr-analyst-report-watch daily mode로 오늘 한국 증권사 리포트 흐름을 수집하고, 공개 리포트 Top 10을 요약한 뒤 topic별 narrative 변화와 source gap을 analysis-example/kr-reports/report-watch-daily-YYYY-MM-DD.md와 .json으로 써줘.
```
...
```text
$kr-daily-market-news로 오늘 한국 시장/업종 데일리 뉴스 리포트를 블로그 게시용으로 작성해줘. analysis-example/kr-market/daily-news-YYYY-MM-DD.md와 .json을 쓴 다음, $kr-naver-blog-publish를 scheduled mode로 사용해줘.
```
