...
- When `order-backlog-analysis.md` and `order-backlog-data.json` exist, ingest their dated official/proxy basis and artifact paths into a distinct backlog block. Do not flatten contract-maturity proxy amounts into official backlog.
...
- When `order-backlog-analysis.md` exists, ingest its `Order-driven company` classification, official total, year/undated split, final disclosed year, coverage ratio, chart basis, and PNG path into `## Order Backlog Inputs`. Preserve `contract-disclosure-maturity-proxy` and `official-total-only` labels exactly.
