...
const usableBars = (bars, endDate, enforcePriceJump = false) => {
  ...
  for (let i = 1; i < throughEnd.length; i++) {
    ...
    if (enforcePriceJump && (current / previous < 0.6 || current / previous > 1.4)) return { ok: false, reason: 'unresolved-price-jump' };
  }
  return { ok: true };
};
function validate({ universeFile, cacheDir, endDate, startDate = '2016-07-11', enforcePriceJump = false, rankingYears = null }) {
  ...
  const allCoverage = A.coverageLedger(universe, (ticker, snapshot) => {
    ...
    const result = usableBars(checked.get(ticker), requiredDate, enforcePriceJump);
    if (!result.ok) perYearFailures.push({ rankingYear: snapshot.rankingYear, ticker, requiredDate, reason: result.reason });
    return result.ok;
  });
  const coverage = rankingYears ? allCoverage.filter(row => rankingYears.has(row.rankingYear)) : allCoverage;
  ...
  // Per-year checks already record every missing cache file as unavailable for
  // that snapshot.  Returning the global list as well would double-count it
  // and, for a scoped validation, would leak failures from later universes.
  const scopedFailures = rankingYears
    ? perYearFailures.filter(row => rankingYears.has(row.rankingYear))
    : perYearFailures;
  return { startDate, endDate, enforcePriceJump, universeFile, cacheDir, unionCount: checked.size + failures.length, usableCount: checked.size, coverage, failures: scopedFailures, pass };
}
function main() {
  ...
  const enforcePriceJump = process.argv.includes('--enforce-price-jump');
  const rankingYears = arg('--ranking-years', null);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(endDate)) throw new Error('--end-date must be YYYY-MM-DD');
  const report = validate({ universeFile, cacheDir, endDate, startDate, enforcePriceJump, rankingYears: rankingYears ? new Set(rankingYears.split(',').map(Number)) : null });
