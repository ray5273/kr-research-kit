...
// Minervini-style 3/6/12M RS + point-in-time EPS/revenue + KOSPI regime and
// 60-session 18% volatility target.  Membership is determined by execution
// year so the first January trade forcibly moves into the new snapshot.
...
function main() {
  ...
  function select(signalDate, executionDate, forcedAnnualRoll) {
    ...
    for (const r of rows) r.score = r.fundamentalScore === null ? null : .5 * r.rsPct + .5 * r.fundamentalScore;
    const ranked = rows.filter(r => r.score !== null).sort((a, b) => b.score - a.score || b.rs - a.rs), holdings = ranked.slice(0, N);
  ...
  for (let i = 0; i < calendar.length; i++) { const date = calendar[i]; cashoutTerminalSeries(date); const prev = calendar[i - 1] || allCalendar[allCalendar.indexOf(date) - 1], order = orders.get(date); const ki = kospi.findIndex(x => x.date === prev); if (ki >= 199) { const ma = mean(kospi.slice(ki - 199, ki + 1).map(x => x.close)), close = kospi[ki].close; if (close > ma * 1.03) regime = true; else if (close < ma * .97) regime = false; } const rs = equity.slice(-60).map((x, j, a) => j ? x.equity / a[j - 1].equity - 1 : null).filter(x => x !== null); const volScale = rs.length >= 59 ? Math.min(1, .18 / (stddev(rs) * Math.sqrt(252))) : 1; const nextExposure = (regime ? 1 : 0) * volScale; if (order) rebalance(date, order, nextExposure, order.forcedAnnualRoll ? 'annual-universe-roll' : 'weekly-rank'); else if (Math.abs(nextExposure - exposure) > 1e-9) rebalance(date, null, nextExposure, 'regime-or-vol-overlay'); exposure = nextExposure; equity.push({ date, equity: value(date, 'close'), cash, holdings: positions.size, exposure }); }
